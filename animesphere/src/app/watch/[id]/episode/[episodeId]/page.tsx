...
// Generate static params for all anime and their episodes
export function generateStaticParams() {
  const params = [];
  for (const anime of animeData) {
    // We're only generating static pages for the first episode
    // to reduce build time and stay under the limit
    params.push({
      id: anime.id,
      episodeId: "1",
    });
...
export default function EpisodePage({
  ...
  // Ensure valid episode number
  const validEpisode = Math.max(
    1,
    Math.min(episodeId, anime.episodes || 1)
  );
              ...
              {Array.from({ length: anime.episodes }).map((_, i) => {
                ...
                return (
                  <Link
                    ...
                    href={`/watch/${params.id}/episode/${episodeNumber}`}
